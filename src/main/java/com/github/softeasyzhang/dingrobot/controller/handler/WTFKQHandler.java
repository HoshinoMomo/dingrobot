...
public class WTFKQHandler implements BaseHandler {
    ...
    public RobotResponse getResultMessage(String content, String senderId) {
        ...
        if(content.contains("下单")){
            ...
            at.add(AllMember.getAllMembers().get("高岩").getDingNo());
        }else if(content.contains("余额") || content.contains("账单")){
            result = "已经帮@结算、余额侧的同学";
            at.add(AllMember.getAllMembers().get("杨熹").getDingNo());
            at.add(AllMember.getAllMembers().get("王凯萌").getDingNo());
        }else if(content.contains("合作")){
            result = "杰哥帮忙反馈一下";
            at.add(AllMember.getAllMembers().get("唐杰").getDingNo());
        }else if(content.contains("临时单") || content.contains("货的单")){
            result = "感谢反馈,正在找相关的人查看";
            at.add(AllMember.getAllMembers().get("徐展").getDingNo());
        }else if(content.contains("充值")){
            result = "建议先到优配后台查询一下充值状态";
            at.add(AllMember.getAllMembers().get("景启明").getDingNo());
        }else if(content.contains("展示")){
            result = "已收到谢谢";
            at.add(AllMember.getAllMembers().get("徐瑞").getDingNo());
        }else if(content.contains("PC")){
            at.add(AllMember.getAllMembers().get("刘丹").getDingNo());
            at.add(AllMember.getAllMembers().get("景启明").getDingNo());
        }else if(content.contains("司机端")){
            at.add(AllMember.getAllMembers().get("王利").getDingNo());
        }else{
            result = content;
        }
        ...
        //@发送人  @今天值班人
        at.add(senderId);
        at.add(whoIsTurn.getWhoIsTurn().getDingNo());
        return RobotResponse.build(result,at);
    }
}
